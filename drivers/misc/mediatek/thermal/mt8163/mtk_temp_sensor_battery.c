...
struct thermal_dev_params mtktsbattery_sensor_tdp = {
#if defined(CONFIG_THERMAL_abg123)
	.offset = -2000,
	.alpha = 10,
	.weight = 300
#else
...
static int mtktsbattery_probe(struct platform_device *pdev)
{
	...
	platform_set_drvdata(pdev, tmp103);
        ...
        if (tmp103->therm_fw) {
                ...
                tmp103->therm_fw->dev_ops = &mtktsbattery_sensor_fops;
                tmp103->therm_fw->tdp = &mtktsbattery_sensor_tdp;
...
static int __init mtktsbattery_sensor_init(void)
{
	...
	if (ret) {
		pr_err("Unable to register mtktsbattery thermal device (%d)\n", ret);
		return ret;
	}
